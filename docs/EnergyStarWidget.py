# Interactive plot of U.S. buildings with Energy Star scores by various parameters
...
homePath = 'C:/Users/schou/Data/Viz_Blogs/EnergyStarAll/'
allData = pd.read_csv(homePath+'EnergyStarBuildingsForPlot.csv', encoding = "ISO-8859-1", dtype={
    'FacilityName': str,	
...
desc = Div(text=open(homePath+"info.html").read(), width=1500)
...
state = MultiSelect(title="State", value=["California"],
        options=open(homePath+'allStates.txt').read().split('\n'),)
bldgType = MultiSelect(title="Type", value=['BANKING','EDUCATION','FOOD_SALES_SERVICE',
    ...
    options=open(homePath+'allBldgTypes.txt').read().split('\n'),)
